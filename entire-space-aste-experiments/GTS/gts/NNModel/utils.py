...
def score_aspect(predicted, golden, lengths, all_tuple_flags, ignore_index=-1, entire_space=True):
    assert len(predicted) == len(golden)
    golden_set = set()
    predict_set = set()
    for i in range(len(golden)):
        if not entire_space and not all_tuple_flags[i]:
            continue
...
def score_opinion(predicted, golden, lengths, all_tuple_flags, ignore_index=-1, entire_space=True):
    assert len(predicted) == len(golden)
    golden_set = set()
    predict_set = set()
    for i in range(len(golden)):
...
def score_uniontags(args, predicted, golden, lengths, sentence_ids, all_tuple_flags,
                    ignore_index=-1,
                    entire_space=True):
    assert len(predicted) == len(golden)
    golden_set = set()
    ...
    for i in range(len(golden)):
        ...
        # if all_tuple_flags[i] and len(golden_tuple) == 0:
        #     print(sentence_ids[i])

        # if not all_tuple_flags[i] and len(golden_tuple) == 0:
        #     check_count += 1
        #
        # if not entire_space and len(golden_tuple) == 0:
        #     continue
        ...
        real_sentence_num += 1
        if len(golden_tuple) == 0 and len(predicted_tuple) == 0:
            both_zero_count += 1

    print('real_sentence_num: %d' % real_sentence_num)
    print('both_zero_count: %d' % both_zero_count)
    print('check_count: %d' % check_count)
