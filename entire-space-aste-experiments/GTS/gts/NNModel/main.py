...
from gts.NNModel.data import load_data_instances, DataIterator
from gts.NNModel.model import MultiInferRNNModel, MultiInferCNNModel
from gts.NNModel import utils
...
def train(args):
    ...
    for i in range(args.epochs):
        ...
        for j in trange(trainset.batch_count):
            _, sentence_tokens, lengths, masks, aspect_tags, _, tags, _ = trainset.get_batch(j)
...
def eval(model, dataset, args):
    ...
    with torch.no_grad():
        ...
        for i in range(dataset.batch_count):
            sentence_ids, sentence_tokens, lengths, mask, aspect_tags, _, tags, tuple_flags = dataset.get_batch(i)
        ...
        for entire_space in [True, False]:
            ...
            precision, recall, f1 = utils.score_uniontags(args, predictions, labels, all_lengths, all_ids, all_tuple_flags,
                                                          ignore_index=-1, entire_space=entire_space)
            print(args.task+'\tP:{:.5f}\tR:{:.5f}\tF1:{:.5f}'.format(precision, recall, f1))
            if entire_space:
                result['.asote.entire_space'] = (precision, recall, f1)
            else:
                result['.asote.sentence_with_pairs'] = (precision, recall, f1)

            aspect_results = utils.score_aspect(predictions, labels, all_lengths, all_tuple_flags,
                                                ignore_index=-1, entire_space=entire_space)
            opinion_results = utils.score_opinion(predictions, labels, all_lengths, all_tuple_flags,
                                                  ignore_index=-1, entire_space=entire_space)
...
if __name__=='__main__':
    ...
    parser.add_argument('--model_dir', type=str, default=common_path.model_dir,
