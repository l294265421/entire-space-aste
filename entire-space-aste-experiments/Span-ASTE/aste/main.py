...
from aste.evaluation import nereval, LinearInstance, FScore
from aste.utils import Shell, hash_text, update_nested_dict
...
class SpanModelTrainer(BaseModel):
    root: Path
    train_kwargs: dict
    path_config: Path = Path(common_path.project_dir + "/training_config/aste.jsonnet").resolve()
...
def main(
    root=common_path.original_data_dir,
    names=("14lap",),
    seeds=(0,),
    sep=",",
    name_out="results",
